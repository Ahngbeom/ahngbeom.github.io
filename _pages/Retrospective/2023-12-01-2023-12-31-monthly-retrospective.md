...
2023년 12월 한 달간의 회고록입니다. 2023년의 마지막 달로, **공적처방전달 시스템 구축** 및 **청구서 결제 안정화**가 핵심 프로젝트였습니다. 11월에 추가한 청구서 결제 방식을 안정화하고, 약국으로 처방전을 전달하는 공적처방전달 시스템을 구축했습니다. 또한 대규모 DB 스키마 정리 작업을 수행했습니다. 총 49개의 이슈를 처리하고, 26개의 커밋(서버 17개 + 프론트 9개)을 작성했습니다.
...
이번 달도 11월에 이어 **100% 서버 중심 작업**이었습니다. 프론트엔드 커밋이 전혀 없었으며, 모든 작업이 서버 API, DB 스키마, 배치 작업에 집중되었습니다. 이는 공적처방전달 시스템과 DB 정리가 주로 백엔드 작업이었기 때문입니다.
...
이슈 수(49개)는 평균 수준이지만, 22개가 DB 스키마 작업이라는 점이 특징적입니다. 커밋 수(16개)는 적은 편이지만, 각 커밋이 공적처방전달, 청구서 결제, 정산 시스템이라는 중요한 기능을 포함합니다.
