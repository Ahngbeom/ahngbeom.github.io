...
date: "2023-11-30"
tags: [회고, 플라잉닥터, 월간회고]
permalink: /Retrospective/2023-11-01-2023-11-30-monthly-retrospective.html
legacy_url: /Retrospective/2023/monthly/2023-11-01-2023-11-30-monthly-retrospective.html
---
## 개요

2023년 11월 한 달간의 회고록입니다. 이 달의 핵심 프로젝트는 **결제선생 안정화** 및 **청구서 결제 방식 추가**였습니다. 10월에 전면 도입한 결제선생의 다양한 오류를 수정하고, 원격진료와 하이패스 결제를 위한 청구서 결제 방식을 새롭게 추가했습니다. 또한 MySQL 8 업그레이드라는 중요한 인프라 작업을 수행했습니다. 총 45개의 이슈를 처리하고, 36개의 커밋(서버 30개 + 프론트 1개 + 플러터 5개 + DB 스키마 2개)을 작성했습니다.
...
### DB Schema - mobidoc-database (2 commits)

| 날짜 | 내용 |
|------|------|
| 11-20 | [20231120] MOBIDOC DATABASE SCHEMA UPDATE |
| 11-28 | UPDATE mobidoc-database-generator.sh |

### Flutter (5 commits) - 결제선생 앱카드 네이티브 연동

| 날짜 | 내용 |
|------|------|
| 11-02 | [결제선생] Android 앱카드 연동 (3 files, +78/-11) |
| 11-03 | androidChannel.setMethodCallHandler if → when 리팩토링 (1 file, +44/-39) |
| 11-03 | [결제선생] iOS 앱카드 연동 (3 files, +64) |
| 11-03 | [결제선생] iOS KB Pay 앱카드 scheme 추가 (1 file, +1) |
| 11-06 | [결제선생] 페이북 intent URL 소문자 치환 이슈 예외처리 (2 files, +9/-2) |
...
### 6. 결제선생 앱카드 네이티브 연동 (Flutter)

Flutter 앱에서 **결제선생 앱카드 결제**를 네이티브로 연동했습니다.

**주요 작업:**
- **Android 앱카드 연동**: Kotlin MethodChannel을 통한 앱카드 결제 연동
- **iOS 앱카드 연동**: Swift를 통한 앱카드 결제 연동
- **KB Pay 앱카드**: iOS KB Pay scheme 추가
- **코드 개선**: androidChannel if → when 리팩토링
- **예외처리**: 페이북 intent URL 소문자 치환 이슈 해결

**영향:**
- Android/iOS 양 플랫폼에서 앱카드 결제 지원
- 결제선생 모바일 결제 완성도 향상
...
- **Frontend**: 0개
- **Flutter**: 5개
- **DB Schema (mobidoc-database)**: 2개

| 도메인 | 커밋 수 |
|--------|---------|
| 결제선생 안정화 | 15개 (43%) |
| 앱카드 네이티브 연동 (Flutter) | 5개 (14%) |
| 통계/정산 | 5개 (14%) |
| 기타 | 10개 (29%) |
...
이번 달은 **서버 중심 작업과 Flutter 앱카드 연동**이 주요 특징입니다. 서버에서는 결제선생 안정화에 집중하고, Flutter 앱에서는 Android/iOS 양 플랫폼의 앱카드 결제를 네이티브로 연동했습니다. 결제선생 안정화와 앱카드 연동이 동시에 진행되어 결제 시스템의 완성도가 크게 향상되었습니다.
...
이슈 수(45개)는 역대 최다 수준이지만, 대부분 결제선생 관련 버그 수정과 UI 개선이었습니다. 커밋 수(35개) 중 서버 30개는 결제 안정성에 집중되었고, Flutter 5개는 Android/iOS 앱카드 네이티브 연동이라는 중요한 작업을 완료했습니다.
