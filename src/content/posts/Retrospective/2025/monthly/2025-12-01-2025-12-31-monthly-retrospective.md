...
date: "2025-12-31"
tags: [회고, 모비닥, 월간회고]
permalink: /Retrospective/2025-12-01-2025-12-31-monthly-retrospective.html
legacy_url: /Retrospective/2025/monthly/2025-12-01-2025-12-31-monthly-retrospective.html
---
## 개요

2025년 12월 한 달간의 회고록입니다. 연말을 맞아 데이터 정리 작업과 버그 수정, 그리고 병원 홈페이지 구독 서비스라는 주요 신규 기능을 개발한 달이었습니다. 총 13개의 이슈를 처리했으며, 캐싱 전략 개선과 SaaS 비즈니스 모델 구축 등 의미있는 성과를 달성했습니다. 또한 Jenkins CI/CD 파이프라인 인프라 개선 작업(kube 3개 커밋)을 통해 배포 환경을 강화했고, **mobidoc-database에서 n8n Schema Sync Workflow, GitLab CI/CD Trigger, DDL 생성/포맷 자동화, Jenkins MR 자동화** 등 DDL 자동화 파이프라인을 집중 구축했습니다(37개 커밋).
...
| 항목 | 백엔드 (mobidoc-server) | 프론트엔드 (mobidoc-front) | kube(인프라) | DB Migration | DB 스키마 | 합계 |
|------|------------------------|---------------------------|-------------|-------------|-----------|------|
| 총 커밋 수 | 35개 | 36개 | 3개 | 10개 | 37개 | **121개** |
| 변경된 파일 | 333개 | 173개 | 2개 | - | - | **508개** |
...
Jenkins CI/CD 파이프라인 환경을 개선하여 배포 안정성을 강화했습니다.

| 날짜 | 커밋 메시지 | 변경 파일 |
|------|-----------|----------|
| 12-18 | [jenkins] Git 및 MySQL 클라이언트 설치 추가 | jenkins/Dockerfile |
| 12-23 | [jenkins] Git 및 MySQL 클라이언트 설치 추가 | jenkins/Dockerfile |
| 12-23 | fix: jenkins/Dockerfile - Helm 설치 명령어 업데이트 | jenkins/Dockerfile, .gitignore |

**주요 개선 사항:**
- Jenkins 컨테이너에 Git 클라이언트 설치 → 파이프라인 내 Git 작업 지원
- MySQL 클라이언트 설치 → DB 마이그레이션 및 점검 작업 지원
- Helm 설치 명령어 업데이트 → Kubernetes 배포 도구 최신화

#### mobidoc-migration(DB 마이그레이션) - 10개 커밋

**레포지토리:** mobidoc-migration

**주요 작업:**
- [MPT-8749] ALTER TABLE mobidoc_product* (12-05)
- Slack 알림/CI 스크립트 대규모 리팩토링 (12-05)
- GitLab MR 자동화: Python3 파싱, Slack 알림 개선, README 기반 요약 (12-16 x4)
- [MPT-8724] 환자 메모 테이블 독립 분리 및 마이그레이션 (12-29)
- [MPT-8796] mobidoc product category 테이블 (12-29)
- [MPT-8762] consult_seq 인덱스 추가 (12-29)
- Claude Code agents 추가 (12-29)

#### mobidoc-database(DB 스키마) - 37개 커밋

**레포지토리:** mobidoc-database (main)

DDL 자동화 파이프라인 집중 구축 작업을 수행했습니다. n8n Schema Sync Workflow, GitLab CI/CD Trigger, DDL 생성/포맷 자동화, Jenkins MR 자동화 등 DB 스키마 관리를 자동화하는 대규모 인프라 작업입니다.

**n8n Schema Sync Workflow (12-09 ~ 12-15, 8개 커밋):**
- n8n Schema Synchronization Workflow 초기 구축
- GitLab Trigger API 연동, SSH 인증 설정
- Schema Sync를 위한 Git 연동 기반 마련

**DDL Generator 개선 (12-12 ~ 12-18, 10개 커밋):**
- DDL 생성 스크립트 리팩토링
- 포맷 자동화 (CREATE TABLE, VIEW, PROCEDURE 등)
- 디렉토리 구조 정리, 미사용 파일 정리
- .gitignore 최적화

**Jenkins MR 자동화 (12-16 ~ 12-23, 8개 커밋):**
- Jenkins Pipeline에서 GitLab MR(Merge Request) 자동 생성
- MR 템플릿 기반 자동 요약 (Python3 파싱)
- Slack 알림 연동
- MR 자동 승인(Approve) 기능

**GitLab CI/CD & 디버깅 (12-23 ~ 12-29, 11개 커밋):**
- GitLab CI/CD Pipeline Trigger 연동
- validate-sync 스크립트 개발 (스키마 변경 감지)
- 디버깅 및 안정화 다수 커밋
- README 업데이트, 설정 개선
...
**kube(인프라)**
```
12/18 (수): ██ 1개 🔧 Jenkins Dockerfile
12/23 (월): ████ 2개 🔧 Jenkins Dockerfile + Helm
```
...
### 양적 성과
12월은 총 **84개의 커밋**(백엔드 35개 + 프론트엔드 36개 + kube 인프라 3개 + DB Migration 10개)을 통해 **508개의 파일**을 수정하고, **33,490라인의 순증**을 기록한 생산적인 한 달이었습니다. 백엔드와 프론트엔드 전반에 걸쳐 균형잡힌 개발을 진행했으며, 특히 프론트엔드에서는 버그 수정(37.5%)에 집중하여 시스템 안정성을 높였습니다. Jenkins CI/CD 인프라 개선(kube 3개 커밋)으로 배포 환경도 강화했습니다.
